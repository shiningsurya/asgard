...
class DPlot : protected Plotter {
		...
		public:
				...
				void Plot(Filterbank& f, int method) {
						...
						// Axis
						PtrFloat timex = new float[wid];
						PtrFloat freqx = new float[nchans];
						...
						for(int i = 0; i < nsteps; i++, i0+=wid, count++) {
								...
								cpgsci(6);
								cpgline(nchans, xrfi.bandshape, freqx);
								// timeshape
								axis[0] = i * timestep;
								axis[1] = axis[0] + timestep;
								axis[2] = 0.0f;
								...
								cpgsci(6);
								cpgline(wid, timex, xrfi.timeshape);
						}
						delete[] dat;
						delete[] timex;
						delete[] freqx;
